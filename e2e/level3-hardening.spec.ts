...
async function openWorkbench(page: Page) {
  ...
  await expect(page.getByTestId('topology-canvas')).toBeVisible(); await expect(page.getByTestId('analysis-journey')).toBeVisible(); await page.waitForTimeout(150); expect(hydrationErrors).toEqual([]);
...
test('Maintenance Trap template uses the same Change Plan path and can return to baseline', async ({ page }) => {
  await openWorkbench(page); await page.getByTestId('scenario-maintenance-trap').click(); await page.getByTestId('load-plan-template').click();
...
test('Growth Wall template produces minimum-cost G2 proposal without applying base changes', async ({ page }) => {
  await openWorkbench(page); await page.getByTestId('scenario-growth-wall').click(); const baseHash = await page.getByTestId('base-model-hash').textContent(); await page.getByTestId('load-plan-template').click(); await page.getByTestId('analyze-plan').click(); await expect(page.getByTestId('verdict')).toHaveText('FAIL'); await expect(page.getByTestId('evidence-panel')).toContainText('G2');
...
test('Resilience Gap N-1 ranks R2 and replay is explicit while optimizer proposal stays inside plan', async ({ page }) => {
  await openWorkbench(page); await page.getByTestId('scenario-resilience-gap').click(); await page.getByTestId('run-resilience').click(); await expect(page.getByTestId('resilience-status')).toContainText('complete', { timeout: 30_000 }); await expect(page.getByTestId('counterexample-R2')).toContainText('#1 · R2');
  ...
  await page.getByTestId('topology-link-R2').click(); await page.getByTestId('plan-link-outage-R2').click(); await page.getByTestId('analyze-plan').click(); await waitForOptimizer(page); await page.getByTestId('run-optimizer').click(); await expect(page.getByTestId('candidate-proposals')).toContainText('R4', { timeout: 30_000 }); await expect(page.getByTestId('candidate-proposals')).toContainText('R5'); await expect(page.getByTestId('base-model-hash')).toBeVisible();
});
