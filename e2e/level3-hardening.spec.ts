...
test('cancellation and plan/network switching prevent stale N-1 publication', async ({ page }, testInfo) => {
  ...
  await page.getByTestId('run-resilience').click(); await expect(page.getByTestId('cancel-resilience')).toBeVisible(); await page.getByTestId('nav-plans').click(); await page.getByTestId('new-plan').click(); await page.getByTestId('nav-network').click(); await page.waitForTimeout(500); await expect(page.getByTestId('contingency-list')).toHaveCount(0);
