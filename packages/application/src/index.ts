...
export class CollaborativeWorkspaceService {
  ...
  capabilityState(){const p=this.a.getProject(),plan=this.a.getPlan(),a=this.getAnalysis(),current=!!a&&isPlanEvidenceFresh(a.stamp,p,plan),hasViolation=!!(current&&a!.verdict==='FAIL'&&a!.capacity.result.violations.length),ids=new Set(current?a!.capacity.result.violations.map(v=>v.linkId).filter((id):id is string=>!!id):[]),hasUnlockedUpgradeTarget=p.links.some(l=>ids.has(l.id)&&!plan.restrictions.lockedLinkIds.includes(l.id)&&(l.upgradeOptions?.some(o=>o.capacityGbps>l.capacityGbps+1e-9)??false)),adaptiveAllowed=plan.constraints.allowedMitigationActions?.routingChanges??true,proposal=this.proposal(),design=this.a.getDesignState?.()??null,designCurrent=!!design&&isPlanEvidenceFresh(design.stamp,p,plan);return{analysisCurrent:current,hasViolation,proposalPresent:proposal.proposals.length>0,proposalStale:proposal.stale,canDecideProposal:proposal.proposals.length>0&&!proposal.stale,canProposeMitigation:hasViolation&&(hasUnlockedUpgradeTarget||(adaptiveAllowed&&!!this.a.optimizeAdaptiveDesign)),canCompareMitigationVariants:hasViolation&&adaptiveAllowed&&!!this.a.optimizeDesignPareto,designCurrent,designVariantCount:designCurrent?design!.variants.length:0,hasCompleteN1:this.a.getContingencies?.()?.analysis.status==='complete'};}
}
