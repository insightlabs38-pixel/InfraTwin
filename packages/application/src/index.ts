...
import { acceptCandidateChange, addPlanChange, assertValidChangePlan, changePlanEvidenceStamp, changePlanHash, changePlanRevisionStamp, compileChangePlanToScenarioPatch, describePlanChange, discardCandidateProposals, isPlanEvidenceFresh, isPlanRevisionFresh, modelHash, rejectCandidateChange, removePlanChange, setCandidateProposals, setChangePlanStatus, setPlanConstraint, setPlanLinkLocked, setPlanNodeLocked } from '../../model/src/index.ts';
...
export interface WorkspaceVerification { status: 'verified'|'failed'|'partial'|'stale'|'cancelled'; modelHash: string; planHash: string; assumptions: string[]; constraintsChecked: string[]; scenarioCoverage: { tested: number; eligible: number; status: 'complete'|'partial'|'not-required' }; evidenceIds: string[]; candidateVerification?: CandidateVerification }
...
export class CollaborativeWorkspaceService {
  ...
  async generateMitigation(signal?:AbortSignal,actor:PlanActor='agent'){if(!this.a.optimizeCapacity)throw new Error('Capacity optimizer is unavailable.');checkAbort(signal);const p=clone(this.a.getProject()),plan=clone(this.a.getPlan()),stamp=changePlanEvidenceStamp(p,plan),r=await this.a.optimizeCapacity(p,this.requirements(),signal);checkAbort(signal);if(!isPlanEvidenceFresh(stamp,this.a.getProject(),this.a.getPlan()))throw new Error('Stale optimizer result discarded because the Change Plan changed.');if(!r.candidate){this.a.publishCandidate?.(null);return null;}const next=setCandidateProposals(this.a.getProject(),this.a.getPlan(),r.candidate,this.now());this.publishPlan(next,false);this.a.publishCandidate?.(r.candidate);this.activity(actor,'proposed_mitigation',`Proposed ${r.candidate.commands.length} mitigation change(s)`,r.candidate.id);return{candidateId:r.candidate.id,proposalCount:r.candidate.commands.length,objective:r.candidate.objective.value};}
  acceptProposalChange(id:string,actor:PlanActor='human'){const next=retag(acceptCandidateChange(this.a.getPlan(),id,this.now()),actor,'accepted_proposal');this.publishPlan(next);this.activity(actor,'accepted_proposal',`Accepted proposal ${id}`,id);return next;}
