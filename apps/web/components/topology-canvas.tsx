              ...
              {snapshot.links.map((link) => {
                ...
                return (
                  <g key={link.id} className="link-group" onClick={(event) => { event.stopPropagation(); selectLink(link.id, event); }}>
                    <line x1={source.x} y1={source.y} x2={target.x} y2={target.y} className={classes} data-testid={`topology-link-${link.id}`} style={{ strokeWidth: clamp(1.4 + utilization / 48 + (priority ? 1.5 : 0), 1.4, 7) }} vectorEffect="non-scaling-stroke" />
