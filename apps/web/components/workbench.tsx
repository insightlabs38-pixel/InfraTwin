...
export function Workbench() {
  ...
  const executeContingencies = async (options: ContingencyRunOptions = {}): Promise<ContingencyAnalysis> => {
    ...
    const next = await runLinkContingenciesAsync(baseProject, basePatch, {
      ...
      onProgress: (value) => { setProgress(value); externalProgress?.(value); },
    });
    ...
    <main className="shell">
      <header className="topbar">
        ...
        <div className="header-actions">
          ...
          <button onClick={resetDemo}>Reset demo</button><button onClick={exportProject}>Export JSON</button><button onClick={() => fileInputRef.current?.click()}>Import JSON</button>
      ...
      <section className="workbench-grid">
        <aside className="panel scenario-panel">
          ...
          <div className="scenario-list">
            {demoScenarios.map((item) => <button key={item.id} className={`scenario-card ${selectedScenarioId === item.id ? 'active' : ''}`} onClick={() => loadDemo(item.id)}><strong>{item.title}</strong><small>{item.kind === 'blank' ? 'Secondary' : item.kind}</small></button>)}
          </div>
          <div className="scenario-story"><strong>{definition.title}</strong><p>{definition.description}</p></div>
          <div className="workflow-actions">
            <button onClick={() => { setActivePatch(null); setSelectedEvidence(null); setGrowth(null); setContingencies(null); setBottleneck(null); }}>Run baseline</button>
        ...
        <article className="panel graph-panel">
          <div className="panel-heading"><div><p className="eyebrow">Topology + active diff</p><h2>Evidence maps to stable graph IDs</h2></div><span className="hint">Click a link for a canonical human edit</span></div>
          {snapshot.nodes.length === 0 ? <div className="empty-canvas"><strong>Blank project</strong><p>Import a valid InfraTwin JSON model to populate the workspace.</p></div> : (
            <svg className="topology" viewBox="0 0 700 455" role="img" aria-label={`${project.name} network topology`}>
              {snapshot.links.map((link) => {
                ...
                const utilization = analysis.routing.linkUtilizationPct[link.id] ?? 0; const disabled = link.available === false; const overloaded = utilization > 100; const high = utilization > 80;
                const selected = selectedLinkIds.has(link.id); const candidateLink = candidateLinkIds.has(link.id); const canonical = project.links.find((item) => item.id === link.id);
                return <g key={link.id} className="link-group" onClick={() => humanToggleLink(link.id)} tabIndex={0} role="button" aria-label={`${link.id} ${disabled ? 'disabled' : `${pct(utilization)} utilized`}`}
                  onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); humanToggleLink(link.id); } }}>
                  <line x1={source.x} y1={source.y} x2={target.x} y2={target.y} style={{ strokeWidth: Math.min(12, 4 + utilization / 18) }} className={`link-line ${disabled ? 'disabled' : overloaded ? 'overloaded' : high ? 'high' : ''} ${selected ? 'selected' : ''} ${candidateLink ? 'candidate-link' : ''}`} />
                  <text x={((source.x ?? 0) + (target.x ?? 0)) / 2} y={((source.y ?? 0) + (target.y ?? 0)) / 2 - 10} className="link-label">{link.id} · {disabled ? 'OFF' : `${pct(utilization)} · ${link.capacityGbps}G`}</text>
              ...
              {snapshot.nodes.map((node) => <g key={node.id} transform={`translate(${node.x ?? 0} ${node.y ?? 0})`}><circle r="31" className={`node-circle ${node.available === false ? 'disabled-node' : ''}`} /><text y="4" textAnchor="middle" className="node-id">{node.id}</text><text y="49" textAnchor="middle" className="node-name">{node.name}</text></g>)}
            </svg>
          )}
          <div className="legend" aria-label="Graph legend"><span><i className="legend-line normal" />normal</span><span><i className="legend-line thick" />loaded</span><span><i className="legend-line dashed" />failed / candidate</span><span><i className="legend-line selected" />evidence / min-cut</span></div>
        </article>
        ...
        <aside className="panel evidence-panel">
          ...
          <dl className="metrics evidence-metrics"><div><dt>Verdict</dt><dd>{analysis.result.verdict}</dd></div><div><dt>Peak</dt><dd>{pct(peak)}</dd></div><div><dt>Unrouted</dt><dd>{analysis.routing.unroutedDemandIds.length}</dd></div><div><dt>Runtime</dt><dd>{analysis.result.runtimeMs} ms</dd></div></dl>
          {growth && <div className="evidence-block"><span className="block-label">Growth evidence</span><strong>First failure: {growth.firstFailureMultiplier ? `${growth.firstFailureMultiplier}×` : 'none'}</strong><p>{growth.firstFailureLinkId ? `${growth.firstFailureLinkId} is the first constraint; target peak ${pct(growth.target.routing.peakUtilizationPct)}.` : 'No failure in tested range.'}</p></div>}
          {contingencies && <div className="evidence-block"><span className="block-label">Resilience evidence</span><strong>{contingencies.completedScenarios}/{contingencies.totalEligibleScenarios} single-link failures · {contingencies.executionMode}</strong><p>Worst: {String(contingencies.result.metrics.worstLinkId)} · score {String(contingencies.result.metrics.worstScore)} · peak {pct(Number(contingencies.result.metrics.worstPeakUtilizationPct))}</p></div>}
      ...
      <section className="lower-grid">
        <article className="panel demand-panel">
          ...
          <div className="demand-list">{project.demands.length === 0 ? <p className="empty">No demands in this project.</p> : project.demands.map((demand) => {
            ...
            return <div className="demand-row" key={demand.id}><button className="demand-route" onClick={() => setSelectedEvidence({ type: 'route', id: `route:${demand.id}`, demandId: demand.id, linkIds: links })}><strong>{demand.id} · {demand.name ?? demand.id}</strong><small>{route?.reachable ? `${route.paths.length || 1} equal-cost path(s) · ${links.join(' / ') || 'local'}` : 'unreachable'} · {serviceClass?.name ?? demand.serviceClassId} ≤ {serviceClass?.maxUtilizationPct ?? 100}%</small>{effectiveDemand.bandwidthGbps !== demand.bandwidthGbps && <em>scenario: {gbps(effectiveDemand.bandwidthGbps)}</em>}</button><span className="number-control"><input aria-label={`${demand.id} bandwidth Gbps`} type="number" min="0" step="1" value={demand.bandwidthGbps} onChange={(event) => humanUpdateDemand(demand.id, Number(event.target.value))} /><em>Gbps</em></span></div>;
          })}</div>
        </article>
