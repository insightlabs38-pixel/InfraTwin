...
export function useWorkbenchStage5(scope: any) {
  const { project, plan, publishedPlanAnalysis, analysisFresh, analysis, progress, compiledPlanPatch, selectedCanonicalLink, snapshot, selectedCanonicalDemand, routeByDemand, settingsLinkId, optimizerStatus, optimizerMessage, designState, workspaceService, candidateStale, pendingProposals, analysisStatus, resilienceStatus } = scope;
  ...
  const canCompareDesignVariants = analysisFresh && publishedPlanAnalysis?.verdict === 'FAIL' && (plan.constraints.allowedMitigationActions?.routingChanges ?? false);
  const capabilityState = workspaceService.capabilityState();
  const canRunMitigation = capabilityState.canProposeMitigation;
  const mitigationDisabledReason = optimizerStatus === 'loading' ? 'Optimizer is still loading.' : optimizerStatus === 'running' ? 'A mitigation run is already in progress.' : !analysisFresh ? 'Analyze the current ChangePlan first.' : publishedPlanAnalysis?.verdict !== 'FAIL' ? 'Mitigation is available after current analysis finds a modeled violation.' : !canRunMitigation ? 'No permitted unlocked mitigation action is available under the current human constraints.' : '';
  const n1DisabledReason = canRunResilience ? '' : 'No available links are eligible for contingency analysis.';
  const workflowGuidance = analysisStatus === 'running' ? 'Analyzing the current ChangePlan…' : optimizerStatus === 'running' ? optimizerMessage : resilienceStatus === 'running' ? 'Testing bounded N-1 contingencies…' : authority === 'DRAFT' ? 'Add a planned change, then analyze.' : authority === 'STALE' ? 'Plan changed — analyze again before trusting prior evidence.' : authority === 'PASS' ? 'Current plan passes modeled checks. Review evidence or run N-1.' : candidateStale ? 'Proposal is stale after the latest plan edit — find mitigation again.' : pendingProposals.length ? `${pendingProposals.length} proposal change${pendingProposals.length === 1 ? '' : 's'} awaiting human review.` : selectedDesignVariant ? `Verified proposal available · peak ${pct(selectedDesignVariant.peakUtilizationPct)} · awaiting human review.` : 'Inspect the failure evidence or find a mitigation.';
