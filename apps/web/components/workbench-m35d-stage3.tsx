...
export function useWorkbenchStage3(scope: any) {
  ...
  const workspaceService = useMemo(() => new CollaborativeWorkspaceService({
    ...
    optimizeDesignPareto: async (base, requirements, signal, sourcePlanHash) => { setOptimizerStatus('running'); setOptimizerMessage('Generating verified adaptive alternatives…'); const variants = await optimizeDesignParetoInBrowser(base, requirements, 10_000, signal, sourcePlanHash, setOptimizerMessage); setOptimizerStatus('ready'); setOptimizerMessage(`${variants.length} verified adaptive alternative${variants.length === 1 ? '' : 's'} available.`); return variants; },
  ...
  const editUpgradeCatalog = (linkIds: string[], options: import('@infratwin/model').LinkUpgradeOption[]) => requestPlanReset('Update the base network?', 'Changing the upgrade catalog changes the base network design options and starts a fresh ChangePlan.', 'Update catalog', () => { try { const nextProject = applyUpgradeProfile(projectRef.current, linkIds, options); setSelectedScenarioId('imported'); replaceBaseProject(nextProject, `${nextProject.name} change plan`); setImportMessage(`Updated the base-network upgrade catalog for ${linkIds.length} link${linkIds.length === 1 ? '' : 's'}. A fresh ChangePlan was started because the base model changed.`); } catch (error) { setImportMessage(error instanceof Error ? error.message : 'Upgrade catalog edit failed.'); } });
  const loadNetworkTemplate = (id: BundledScenarioId) => requestPlanReset('Switch networks?', 'Opening another bundled network replaces the current network and ChangePlan.', 'Switch network', () => { const next = loadScenario(id); setSelectedScenarioId(id); replaceBaseProject(next, `${getScenarioDefinition(id).title} change plan`); setImportMessage(''); });
  const loadPlanTemplate = () => { if (!definition.changePlanTemplate) return; requestPlanReset('Load the example plan?', 'Loading the example replaces the current ChangePlan for this network.', 'Load example', () => { clearAllDerived(); const next = clonePlan(definition.changePlanTemplate!); planRef.current = next; setPlan(next); }); };
  const createFreshPlan = (name: string) => { clearAllDerived(); const next = createChangePlan(projectRef.current, name, { id: `plan-${projectRef.current.id}-${++planCounterRef.current}` }); planRef.current = next; setPlan(next); };
  const newPlan = (name: string) => requestPlanReset('Start a new ChangePlan?', 'Starting a new ChangePlan discards the current plan state.', 'Start new plan', () => createFreshPlan(name));
  const clearPlan = () => requestPlanReset('Clear this ChangePlan?', 'Clearing removes planned changes, restrictions, and proposals from this ChangePlan.', 'Clear plan', () => createFreshPlan(planRef.current.name));
