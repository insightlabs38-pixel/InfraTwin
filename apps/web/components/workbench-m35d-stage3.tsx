...
export function useWorkbenchStage3(scope: any) {
  ...
  const workspaceService = useMemo(() => new CollaborativeWorkspaceService({
    ...
    optimizeAdaptiveDesign: async (base, requirements, signal, sourcePlanHash) => { setOptimizerStatus('running'); setOptimizerMessage('Exploring bounded candidate routes and declared design actions off the main thread…'); const result = await optimizeAdaptiveDesignInBrowser(base, requirements, 10_000, signal, sourcePlanHash); setOptimizerStatus('ready'); setOptimizerMessage(result.variant ? `${result.variant.verification.status} adaptive design · cost ${result.variant.totalCost} · peak ${Math.round(result.variant.peakUtilizationPct * 10) / 10}%` : `${result.failureReason ?? result.diagnostics.status} · ${result.diagnostics.message}`); return result; },
    optimizeDesignPareto: async (base, requirements, signal, sourcePlanHash) => { setOptimizerStatus('running'); setOptimizerMessage('Generating a bounded nondominated adaptive-design frontier…'); const variants = await optimizeDesignParetoInBrowser(base, requirements, 10_000, signal, sourcePlanHash); setOptimizerStatus('ready'); setOptimizerMessage(`${variants.length} verified nondominated adaptive design variant(s).`); return variants; },
