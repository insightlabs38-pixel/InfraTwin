...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| F-001 | Construct two semantically different Level 4B topology states with the same 32-bit topology digest, prime cache with topology A, then request topology B | Level 4B graph/path cache | Cache cannot reuse semantically invalid path data | Cache identity must distinguish all route-semantic states even if a display digest collides | **FIXED:** baseline used its 32-bit FNV digest as cache authority. A concrete pair of directed topologies collides at legacy `fnv1a32:cf42e5c3`; one has A→B→C reachability and the other has no A→C path. Cache authority now uses exact canonical structural encoding while FNV remains diagnostics only. | **P1 resolved** | `3340243d7c1f7a2c746edcf5925c95a3ced389a5` | `tests/final-adversarial.test.ts` at `fb85bb392ae596350f42b72f472c0c35ad6cbc64`; passes in focused WebMCP hardening run together with the full unit suite |
| F-002 | Native WebMCP state refreshes overlap while dynamic capabilities are registering | WebMCP dynamic registration / Strict Mode | Revoked/current capabilities only; no duplicate native registrations | Registration refresh must be serialized/owned and must not emit duplicate tool registrations or unhandled rejections | **FIX IMPLEMENTED / NATIVE RETEST PENDING:** baseline native Chromium logs contained repeated unhandled `InvalidStateError: Duplicate tool name`. Refreshes are now serialized inside the registration owner; failed UI-triggered refreshes are caught/exposed. Deterministic 8-way concurrent refresh regression passes. | **P1 pending native closure** | `b33c5ee6de5031bb3b9615eb7098b4b11ad001b0` | `AV-21/F-002` in `tests/phase35d-webmcp-coactivity.test.ts`; native E2E now explicitly fails on duplicate registration errors |
...
| AV-05 | Safe Level 4B reuse: bandwidth/budget/utilization/Pareto target | Path cache | Reuse must equal cache-disabled/reference execution | Existing Level 4B differential/reuse suite passes; final matrix audit pending | — | — | `tests/level4b-path-engine.test.ts` |
| AV-05B | Unsafe reuse attempts: link/node outage, weight, forbidden link/node, candidate add/remove/weight, topology replacement, direction | Path cache | Every route-semantic change must invalidate | F-001 cache authority fixed; existing invalidation suite plus collision regression pass; delimiter/key-completeness audit pending | — | `3340243d7c1f7a2c746edcf5925c95a3ced389a5` | `tests/level4b-path-engine.test.ts`, `tests/final-adversarial.test.ts` |
| AV-06 | Cache collision / key completeness including delimiter-like IDs, K/diversity, candidate semantics and routing restrictions | Path cache | No two route-semantic states may share cache identity | F-001 collision class closed by exact JSON cache authority; remaining key-completeness cases pending | P1 resolved for collision class | `3340243d7c1f7a2c746edcf5925c95a3ced389a5` | `tests/final-adversarial.test.ts` |
...
| AV-11 | Create proposal then mutate plan/constraint/lock/restriction/catalog/candidate options and attempt direct acceptance | Proposal lifecycle | Semantically stale proposal becomes undecidable and direct acceptance fails | Code audit confirms direct acceptance checks `sourcePlanHash` against current semantic plan hash; executable expansion pending | — | — | existing Phase 3.5A/Level 3 stale-proposal tests |
...
| AV-21 | Rapid no-analysis→FAIL→edit→FAIL→proposal→stale transitions | Dynamic WebMCP capability lifecycle | Exactly-once registration; revoked tools disappear; no duplicate/old registrations; revoked execution fails | Deterministic concurrent-refresh race closed; native proof pending | P1 pending native closure | `b33c5ee6de5031bb3b9615eb7098b4b11ad001b0` | `AV-21/F-002` concurrent refresh regression + native lane |
| AV-22 | Native Strict Mode mount/unmount/remount | Native WebMCP | Zero duplicate registrations; cleanup guaranteed; current origin cluster correct | FIX implemented; headed native retest pending | P1 pending native closure | `b33c5ee6de5031bb3b9615eb7098b4b11ad001b0` | `e2e/phase35d-native-webmcp.spec.ts` now captures duplicate-registration errors |
...
| AV-43 | Native Chromium: malicious label, agent mutation+human override, revoked execution, lock→adaptive, stale verification | Native WebMCP red team | Same safety invariants hold on actual host, not only mock harness | F-002 must close first | P1 pending | pending | native red-team expansion pending |
